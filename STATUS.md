...
- [ ] **2.6** Point one test agent at `worker` — **You** — adapter `gemini_local`, engine **`cli`**. Headless auth/trust applied (PR #5). Next: LiteLLM ingress was `INTERNAL_ONLY` so Paperclip’s public `*.run.app` call got HTML 404.
